				...
				if (cancelList != null) {
					for (PassengerVO cancelVO : cancelList) {
						...
						if (roomno == 0) {
							...
							roomno = (long) Execute.run(messageRoomFRNService, new Object[] { cancelId, cancelPid });
						}

						// 메시지 발송
						MessageVO vo = new MessageVO();
						vo.setContent("[알림]동승을 신청한 카풀의 운행이 취소되었습니다. 동승이 확정된 상태였다면 운행내역에서 취소내역을 확인하실 수 있습니다.");
						Execute.run(messageWriteService, new Object[] { cancelId, cancelPid, roomno, vo });
						Execute.run(messageRoomIncreaseService, new Object[] { roomno, cancelPid });
					}
				}
